...
int barrier_array[10];
struct sleeplock bar_lk;
struct cond_t cv_bar_arr[10];
// cond
struct buffer_elem buff_arr[20];
int buffer_head;
int buffer_tail;
struct sleeplock buffer_lock;
struct sleeplock buffer_insert_lock;
struct sleeplock buffer_delete_lock;

// sema
int buffer_sem_arr[20];
int buf_sem_head;
int buf_sem_tail;
struct semaphore sem_pro;
struct semaphore sem_com;
struct semaphore sem_empty;
struct semaphore sem_full;
struct sleeplock sem_printlock;
...
void procinit(void)
{
  ...
  for (int buffer__i = 0; buffer__i < 10; buffer__i++)
  {
    barrier_array[buffer__i] = -1;
  }
...
void wakeupone(struct cond_t *chan)
{
  struct proc *p;
  uint xticks;
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    {
      acquire(&p->lock);
      if (p->state == SLEEPING && p->chan == chan && flag == 0)
      {
        p->state = RUNNABLE;
        flag = 1;
        p->waitstart = xticks;
...
int barrier_alloc(void)
{
  ...
  for (i = 0; i < 10; i++)
  {
    if (barrier_array[i] == -1 && flag == 0)
    {
      barrier_array[i] = 0;
      initcond(&cv_bar_arr[i], "barrier_cond");
      ans = i;
      flag = 1;
    }
  }
...
void barrier(int bar_i, int arr_id, int n)
{
  acquiresleep(&bar_lk);
  printf("%d: Entered barrier#%d for barrier array id %d\n", myproc()->pid, bar_i, arr_id);
  barrier_array[arr_id] = barrier_array[arr_id] + 1;
  releasesleep(&bar_lk);

  acquiresleep(&cv_bar_arr[arr_id].lk);
  // printf("%d \n",n);
  if (barrier_array[arr_id] == n)
  {
    barrier_array[arr_id] = 0;
    // printf("brod\n");
    cond_broadcast(&cv_bar_arr[arr_id]);
  }
  else
    cond_wait(&cv_bar_arr[arr_id], &cv_bar_arr[arr_id].lk);
  releasesleep(&cv_bar_arr[arr_id].lk);
...
void buffer_cond_init(void)
{
  buffer_head = 0;
  buffer_tail = 0;
  initsleeplock(&buffer_lock, "buffer_printlock");
  initsleeplock(&buffer_insert_lock, "buffer_insertlock");
  initsleeplock(&buffer_delete_lock, "buffer_deletelock");
  for (int i = 0; i < 20; i++)
  {
    buff_arr[i].x = -1;
    buff_arr[i].full = 0;
    initcond(&buff_arr[i].inserted, "buff_elem_insert");
    initcond(&buff_arr[i].deleted, "buff_elem_delete");
    initsleeplock(&buff_arr[i].lock, "buff_elem_lock");
  }
}

void cond_produce(int i)
{
  int index;
  acquiresleep(&buffer_insert_lock);
  index = buffer_tail;
  buffer_tail = (buffer_tail + 1) % 20;
  releasesleep(&buffer_insert_lock);

  acquiresleep(&buff_arr[index].lock);
  while (buff_arr[index].full)
    cond_wait(&buff_arr[index].deleted, &buff_arr[index].lock);
  buff_arr[index].x = i;
  buff_arr[index].full = 1;
  cond_signal(&buff_arr[index].inserted);
  releasesleep(&buff_arr[index].lock);
}

int cond_consume(void)
{
  int index, out = -1;
  acquiresleep(&buffer_delete_lock);
  index = buffer_head;
  buffer_head = (buffer_head + 1) % 20;
  releasesleep(&buffer_delete_lock);

  acquiresleep(&buff_arr[index].lock);
  while (!(buff_arr[index].full))
    cond_wait(&buff_arr[index].inserted, &buff_arr[index].lock);
  out = buff_arr[index].x;
  buff_arr[index].full = 0;
  cond_signal(&buff_arr[index].deleted);
  releasesleep(&buff_arr[index].lock);

  acquiresleep(&buffer_lock);
  printf("%d ", out);
  releasesleep(&buffer_lock);
  return out;
}
...
void buffer_sem_init(void)
{
  sem_init(&sem_pro, 1);
  sem_init(&sem_com, 1);
  sem_init(&sem_empty, 20);
  sem_init(&sem_full, 0);
  initsleeplock(&sem_printlock, "sem_printlock");
  buf_sem_head = 0;
  buf_sem_tail = 0;
}

void sem_produce(int i)
{
  sem_wait(&sem_empty);
  acquiresleep(&sem_printlock);
  sem_wait(&sem_pro);
  buffer_sem_arr[buf_sem_tail] = i;
  buf_sem_tail = (buf_sem_tail + 1) % 20;
  sem_post(&sem_pro);
  releasesleep(&sem_printlock);
  sem_post(&sem_full);
}
...
int sem_consume(void)
{
  ...
  acquiresleep(&sem_printlock);
  sem_wait(&sem_com);
  out = buffer_sem_arr[buf_sem_head];
  buf_sem_head = (buf_sem_head + 1) % 20;
  sem_post(&sem_com);
  releasesleep(&sem_printlock);
  sem_post(&sem_empty);
  acquiresleep(&sem_printlock);
